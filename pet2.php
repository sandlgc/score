...
while ($p = $pets->fetch_assoc()) {
    ...
    $new_hunger = max(10, $hunger - $total_hunger);
    $new_mood = max(10, $mood - $total_mood);
...
async function confirmRareUnlock(e) {
    ...
    const res = await fetch('pet2.php', {
        method: 'POST',
        body: formData
    });
...
async function startPk(mid){
    ...
    // 每次PK都重新获取最新题目，永不失效
    try {
        const res = await fetch("pet2.php?get_question=1");
...
async function refreshMyCardsPanel() {
    try {
        const response = await fetch('pet2.php?refresh_cards_data=1');
...
async function refreshPetDataAfterPK() {
    const userRes = await fetch('pet2.php', {
        method: 'POST',
    ...
    for (const el of petElements) {
        ...
        try {
            ...
            if (data.code === 1) {
                ...
                if (data.is_sick) {
                    document.getElementById(`pet-sick-${pid}`).style.display = 'block';
                    document.getElementById(`btn-cure-${pid}`).style.display = 'block';
                    document.getElementById(`btn-normal-${pid}`).style.display = 'none';
                } else {
                    document.getElementById(`pet-sick-${pid}`).style.display = 'none';
                    document.getElementById(`btn-cure-${pid}`).style.display = 'none';
                    document.getElementById(`btn-normal-${pid}`).style.display = 'block';
                }
